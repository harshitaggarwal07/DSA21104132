...
/* BONUS QUESTION : Try to find a way to link the family members' doubly-linked list based on their relationship. (Is it possible?)
ANSWER : one way is to arrange them according to there age but then again there may be some expection for eg if a father divorced his first wife 
and his second wife is younger than his son then order might not be correct also if a parent has more than one children then too it would become 
messy and we will have to create multiple double linked list hence other types of data structures would be more suited in this case 
*/
